...
void usage(char* exe)
{
    ...
    std::cout << "\t" << exe << " <config.ini> create_app -name <app_name> -type <app_type> [-pc partition_count] [-rc replication_count] [-url package_id] [-stateless]" << std::endl;
...
int main(int argc, char** argv)
{
    ...
    for(int index = 3; index < argc; index++)
    {
        ...
        else if (strcmp(argv[index], "-url") == 0 && argc > index)
        {
